...
const desktopNavClass = (active: boolean) =>
  cn(
    ...
    active ? "text-[#C9A962]" : "text-white hover:text-white/80",
  );

const mobileNavClass = (active: boolean) =>
  cn(
    "border-b border-white/10 py-3.5 text-base transition-colors",
    active
...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/5 bg-[#a5a196]/90 text-white backdrop-blur-md">
      <div className="mx-auto flex h-16 w-full max-w-[1400px] items-center justify-between gap-3 px-4 sm:h-[4.5rem] sm:px-6 md:h-20 md:px-8 lg:grid lg:h-24 lg:grid-cols-[auto_1fr_auto] lg:items-center lg:gap-6 lg:px-10 xl:gap-8">
        ...
        <div className="relative z-10 flex shrink-0 items-center justify-end gap-1 sm:gap-2 md:gap-3 lg:justify-self-end">
          ...
          <button
            type="button"
            onClick={openSearch}
            className="inline-flex shrink-0 p-1.5 text-white transition-colors hover:text-white/80"
            aria-label={t("search")}
          >
          ...
          <Link
            href={accountHref}
            className="hidden shrink-0 p-1.5 text-white transition-colors hover:text-white/80 md:inline-flex"
          ...
          <button
            type="button"
            onClick={openCart}
            className="relative hidden shrink-0 cursor-pointer p-1.5 text-white transition-colors hover:text-white/80 sm:inline-flex"
          ...
          <button
            type="button"
            className="inline-flex shrink-0 p-1.5 text-white transition-colors hover:text-white/80 lg:hidden"
      ...
      {mobileOpen ? (
        <div className="max-h-[calc(100svh-4rem)] overflow-y-auto border-t border-white/10 bg-[#a5a196]/98 px-4 py-5 text-white backdrop-blur-md sm:px-6 lg:hidden">
            ...
            {navigation.map((item) =>
              hasChildren(item) ? (
                <div key={item.key} className="border-b border-white/10">
                  ...
                  {mobileShopOpen ? (
                    <div className="mb-2 ms-1 space-y-0.5 border-s border-white/15 ps-3">
                      {item.children.map((child) => {
                        const childActive = isShopChildActive(
                          pathname,
                          searchParams,
